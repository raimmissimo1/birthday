...
export function GiftScene({ gift, preset, lit, stage, onEntranceComplete, activeCardId, onToggleCard }: Props) {
  ...
  return <><group ref={group} position={[0, ENTRANCE_HEIGHT, 0]}><Table />{gift.frames.map(({ id, image, position, rotation, scale }) => <PictureFrame key={id} image={image} position={position} rotation={rotation} scale={scale} />)}{gift.cards.map((card) => <BirthdayCard key={card.id} id={card.id} image={card.image} tablePosition={card.position} tableRotation={card.rotation} isActive={activeCardId === card.id} onToggle={onToggleCard} />)}<ProceduralCake preset={preset} candleLit={lit} /></group><Fireworks isActive={stage === "finale"} origin={[0, 3, -3]} /></>;
}
