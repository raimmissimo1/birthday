const berries = [[-0.42, 0.12], [-0.28, 0.28], [-0.18, 0.06], [0.45, -0.2], [0.55, -0.08]] as const;

const pearls = [0.45, 2.55, 3.4, 5.25];
...
export function RomanticCakeDecorations({ topY }: { topY: number }) {
  return (
    <group>
      {berries.map(([x, z], index) => (
        <group key={`${x}-${z}`} position={[x, topY + 0.085, z]}>
          <mesh castShadow>
            ...
            <meshStandardMaterial color="#b93666" roughness={0.32} metalness={0.02} />
          </mesh>
      ...
      {([[-0.55, -0.18], [-0.05, 0.52], [0.5, 0.23]] as const).map(([x, z]) => (
